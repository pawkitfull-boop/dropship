        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {CARDS.map((card, idx) => (
            <Link 
              key={idx} 
              href={card.href}
              className={`group flex flex-col justify-between p-6 sm:p-8 rounded-3xl ${card.bgColor} min-h-[220px] sm:min-h-[300px] transition-transform hover:-translate-y-2 hover:shadow-xl`}
            >
              <div>
                <h3 className="text-2xl sm:text-3xl font-bold text-black leading-tight mb-4">
                  {card.problem}
                </h3>
              </div>
